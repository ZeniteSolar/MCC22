...
def get_averaged_large_signal_model(ss_on: lca.StateSpace, ss_off: lca.StateSpace) -> lca.StateSpace:
    dt = TimeDomainExpression(f'd(t)')
...
def get_averaged_small_signal_model(ss_on: lca.StateSpace, ss_off: lca.StateSpace) -> lca.StateSpace:
    dt = TimeDomainExpression(f'd(t)')
...
def lcapy_statespace_to_pythoncontrol(ss: lca.StateSpace) -> control.StateSpace:
    return control.StateSpace(np.array(ss.A.expr, dtype=float),
...
def lcapy_statespace_to_scipy(ss: lca.StateSpace) -> control.StateSpace:
    return signal.StateSpace(np.array(ss.A.expr, dtype=float),
                             np.array(ss.B.expr, dtype=float),
                             np.array(ss.C.expr, dtype=float),
                             np.array(ss.D.expr, dtype=float))

def get_all_symbols(ss: lca.StateSpace) -> dict:
    symbols_dict = {}
    for matrix in [ss.A, ss.B, ss.C, ss.D, ss.u, ss.x, ss.y]:
        for k,v in matrix.symbols.items():
            if k not in symbols_dict.keys():
                symbols_dict[k] = v
    return symbols_dict
